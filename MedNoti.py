...
def set_alarms(alarm_times, medicine_name, amount, description, user_uid):
    for time_str in alarm_times:
        # Convert the time_str to a datetime object
        alarm_time = datetime.strptime(time_str, "%H:%M").replace(year=datetime.now().year, month=datetime.now().month, day=datetime.now().day)
        
        # Save the alarm to Firestore
        save_alarms_to_db(user_uid, medicine_name, amount, description, alarm_times)
        
        # Example: You can call the check_time function for each alarm time
...
def view_running_alarms(user_uid):
    active_alarms = get_active_alarms(user_uid)
    if not active_alarms:
        print("No active alarms found.")
    else:
        for alarm in active_alarms:
            print(f"\nMedicine: {alarm['medicine_name']}")
            print(f"Amount: {alarm['amount']} biji")
            print(f"Description: {alarm['description']}")
            print(f"Alarm Times: {', '.join(alarm['alarm_times'])}")
